...
app.use(cors());
app.use(express.json({ limit: '10mb' }));           // ← изменено
app.use(express.urlencoded({ limit: '10mb', extended: true }));  // ← добавлено
...
app.post('/api/employees', (req, res) => {
    const { name, position, department_id, salary, email, phone, hire_date, bio, photo } = req.body;

    const query = `
        INSERT INTO employees 
        (name, position, department_id, salary, email, phone, hire_date, bio, photo) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;

    db.run(query, [name, position, department_id, salary, email, phone, hire_date, bio, photo], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        res.status(201).json({ id: this.lastID });
    });
});

// 3. PUT — Редактирование
app.put('/api/employees/:id', (req, res) => {
    ...
    const query = `
        UPDATE employees 
        SET name = ?, position = ?, department_id = ?, salary = ?, 
            email = ?, phone = ?, hire_date = ?, bio = ?, photo = ? 
        WHERE id = ?
    `;

    db.run(query, [name, position, department_id, salary, email, phone, hire_date, bio, photo, id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
